...
class ModTimeResponse implements Response {
    ...
    public void process(final LoggedDataInputStream dis, final ResponseServices services) throws ResponseException {
        try {
            final String dateString = dis.readLine();
            ...
            if (date.getTime() < 0) {
                ...
                if ((date.getYear() < 100) && (date.getYear() >= 70)) {
                     ...
                     * "Cannot adjust negative time value (" + dateString + ")",
                     * //NOI18N
                     * ResponseException.getLocalMessage("ModTimeResponse.badDate"
                     * , //NOI18N new Object[] {dateString}));
                     */
                }
            }
